...
int circular_buffer_write(struct logfs* logfs, const void* data, size_t len) {
    ...
    if (end_space>len) {
       ...
       /*  Copy in two steps (split over the end of the buffer) */
...
int circular_buffer_read(struct logfs* logfs,size_t len) {
    ...
    device_write(logfs->device,(char*)buffer->buffer+buffer->tail,offset,len);
...
int circular_buffer_flush(struct logfs* logfs,size_t len) {
    ...
    if (buffer->tail+len*2<=buffer->capacity){
  
    device_write(logfs->device,(char*)buffer->buffer+buffer->tail,offset,len*2);
    }
    else{
    device_write(logfs->device,(char*)buffer->buffer+buffer->tail,offset,len);
    device_write(logfs->device,(char*)buffer->buffer,offset+len,len);


    }
   /*  buffer->tail=(buffer->tail+len)%buffer->capacity;
    offset=offset+len;
...
void* consumer(void* arg) {
...

    {
        ...
        if(logfs->write_cache->size<len){pthread_mutex_unlock(&logfs->mutex);}
        
        else
        {


        f = circular_buffer_read(logfs,len);
        if (0!=f){
        TRACE("can't write");
        pthread_exit(NULL);
        }
...
struct logfs *logfs_open(const char *pathname) {
     /* Allocate memory for logfs structure */
...
int logfs_read(struct logfs *logfs, void *buf, uint64_t off, size_t len) {
    ...
    /* char * buffer_char; */
    /* buffer_char = logfs->read_cache->buffer; */
  
    fflush(stdout);

     /* flush all the write cache to file */
    ...
    f = circular_buffer_flush(logfs,block_size);
    if (0!=f){
        TRACE("can't write");
        return -1;
    }
    ...
    if(!memcpy(buf, (char*)logfs->read_cache->buffer + buf_new, len)){
